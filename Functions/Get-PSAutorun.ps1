    ...
    Begin
    {
        ...
        Function Get-PSRawAutoRun
        {
            ...
            Begin
            {
                ...
                if (($PSBoundParameters.Keys | Where-Object { $_ -notin "ShowFileHash", "VerifyDigitalSignature" }).Count -eq 0)
                {
                    $All = [switch]::Present
                }
            }
                            ...
                            switch -Regex ($Item.Value)
                            {
                                ...
                                'SysmonDrv.sys' {
                                    $env:PATH -split ';' | ForEach-Object {
                                        Get-ChildItem -Path $_\*.sys -Include SysmonDrv.sys -Force -ErrorAction 0
